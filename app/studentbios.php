...
require_once 'include/protect.php'; //make sure user is logged in
...
$checkstudent = $StudentDAO->getStudent($_SESSION['userid']); //check if user is student, not admin
...
#use variable activetab to check which tab to show
...
# sets default tab; make sure one active tab only
...
$allcourses = array(); #get all avail courses

# get student biddable modules and bids
if ($active != FALSE) { #theres an active bidding round
    $studentbidded = $BidDAO->getBidFromUserByRoundNo($_SESSION['userid'], $active->roundNo); #get bids
    $biddablecourses = $StudentDAO->getStudentBiddableModule($_SESSION['userid'], $active->roundNo); #get biddable courses
...
foreach ($allcourses as $key => $acourse) { #get details of all courses
...
$calendarjson = array(); #store calendar information 
...
foreach ($enrolledcourses as $acourse) {
    ...
    $starttime = $sectioninfo->start; #comes in datetime format
    $endtime = $sectioninfo->end;
    $day = $sectioninfo->day; #comes in numerical format
    if ($day == '7') { #change sunday to 0
        $day = '0';
    }
    $acourse = (array) $acourse; #change class object to array
...
if (isset($studentbidded)) { #gets the bidded courses
...
$_SESSION['calendarjson'] = json_encode($calendarjson); #put in session variable
$biddingcart = array(); #declare empty cart
...
<head>
    ...
    <!-- JS script --> <!-- user defined js file -->
    <script type="text/javascript" src="include/scripts/studentbios.js"></script>

    <script>
        // loads calendar
        document.addEventListener('DOMContentLoaded', function() { //when everything is loaded, run function
            var calendarEl = document.getElementById('calendar'); //define variable calendar E1
            
            var calendar = new FullCalendar.Calendar(calendarEl, {
                plugins: ['interaction', 'dayGrid', 'timeGrid'], //gives additional function
                contentHeight: 'auto',
                height: 'parent', //gets height from div
                defaultView: 'timeGridWeek', //shows in week
                defaultDate: new Date(), //default date is today
                minTime: '08:00:00',
                maxTime: '24:00:00',
                allDaySlot: false,
                slotDuration: '01:00:00', //on
                ...
                events: <?php echo $_SESSION['calendarjson'] ?> //shows events in calendar
            });
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light shadow-sm fixed-top">
        ...
        <ul class="navbar-nav flex-row ml-md-auto d-none d-md-flex"> <!--flex = distribute spaces in the screen-->
    ...
    <div class="container-fluid d-flex flex-column"> 
        <div class="row">
            <div class="col-xs-12 col-12"> <!-- for screen that are extra small -->
                <nav>
        ...
        <div class="row"> <!-- displays in one row-->
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-m-12" id="overview-container">
                                <div class="row pl-3">
                                    <?php
                                    if ($active != FALSE) { //shows datetime of round
                                ...
                                <div class="row pl-3 pr-2">
                                    ...
                                    <div class="card shadow container-fluid px-0">
                                        <div class="card-header" id='availablecourseHeader'> <!--shows header in cart-->
                                        ...
                                        <div id="availableCourse" class="collapse show" aria-labelledby="availablecourseHeader">
                                            <div class="card-body container-fluid p-0 m-0 overflow-auto">
                                                <div class="row p-0 m-0"> <!--p=padding, m=margin-->
                                                    <nav class="container-fluid nav nav-fill p-0 m-0 navbar-sticky-top">
                                                        <div class="container-fluid nav nav-tabs nav-justified p-0" id="nav-tab" role="tablist">
                                                            ...
                                                            foreach ($allschool as $aschool) {
                                                                if ($aschool == $studentsch) { //default school tab is own school
                                                                    echo "
                                                ...
                                                <div class="row p-0 m-0 container-fluid tab-content" id="nav-tabContent"> 
                                                    ...
                                                    foreach ($allschool as $aschool) {
                                                        ...
                                                        $schoolcourses = array(); //get courses in sch
                                                        ...
                                                        if (!$active || ($active->roundNo == '1' && $studentsch != $aschool || count($schoolcourses) == 0)) { //if theres no active round or active, not my sch or no courses in the school
                                                            ...
                                                            <table class='table p-0 m-0'>
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    foreach ($schoolcourses as $key => $value) {
                                                                        ...
                                                                        $acourse = json_encode($value);  //pin to bidding cart
                                                    ...
                                                    <div class="tab-pane fade show flex-grow-1" id="nav-search" role="tabpanel" aria-labelledby="nav-search-tab">
                                                        ...
                                                        <input type="text" id="searchCourse" onkeyup="searchCourse()" placeholder="Search course code.."> <!-- jquery function - searchCourse(), onkeyup = when live searching in search field-->                                                       
                                                        </div>
                                    ...
                                    <div class="card shadow container-fluid px-0">
                                        <div class="card-header" id='biddingCartHeader'>
                                            ...
                                            <div class="d-inline-flex float-right">
                                                ...
                                                <button class="btn m-0 py-0 px-1" <?= $disabled ?> type="button" onclick="checkout()" id="checkoutbtn"><i class="fas fa-shopping-cart"></i> Checkout</button> <!-- JS function = checkout -->
                                                <!-- </form> -->
                                            </div>
                                        </div>
                            ...
                            <div class="col-lg-6 col-m-12 pl-2" id="overview-container"> <!-- change to col-->
                                <div class="card shadow">
                                    ...
                                    <div class="card-body overflow-auto p-0" id="calendarbody">
                                        <div id="calendar"></div> <!-- calendar placeholder/location -->
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row justify-content-center">
                            <div class="col-8 col-m-12 border rounded py-3">
                                ...
                                <div class="row container-fluid m-0 py-3">
                                    <?php
                                    if (isset($_SESSION['deletebidresult'])) { //when delete bid 
                                        ?>
                                        <div class="alert alert-info container-fluid" role="alert"> <!--alert-info = blue colour-->
                                        <?php
                                            if ($_SESSION['deletebidresult']['delete']['status'] == 'success') {
                                                ...
                                                ";//e\ for special char = escape characters
                                            } else {
                                                echo "
                                        ...
                                        if ($active == FALSE) {
                                            ...
                                            if (empty($studentbidded)) {
                                                ...
                                                <table class="table table-bordered">
                                                    <thead class="thead-light text-center">
                                                        <tr>
                                                            ...
                                                            <th>Section</th>
                                                            ';
                                                if ($active->roundNo == '2') { //only displays min bid and vacancy in Round 2
                                                    echo '
                                                            <th>Min Bid</th>
                                                            <th>Vacancy</th>';
                                                }
                                                ...
                                                foreach ($studentbidded as $abid) {                                            
                                                    $biddetails = json_encode($abid); //prepare for JS
                                                    $code = $abid->code;
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="row justify-content-center">
                            <div class="col-8 col-m-12 border rounded py-3">
                                <div class="row justify-content-center">
                                    <?php
                                    if ($BiddingRoundDAO->getLastRound() == '' || ($active && $BiddingRoundDAO->getLastRound() == '1')) { #if no last round or active round but last round is 1, displays no past round
                                        echo '
                                ...
                                <div class="row container-fluid m-0 py-3">
                                <?php //get bids results based on last round
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="row justify-content-center">
                            <div class="col-8 col-m-12 border rounded py-3">
                                ...
                                <div class="row container-fluid m-0 py-3">
                                <?php
                                if (isset($_SESSION['deleteresult'])) { //for delete section
                                    ?>
