...
if ($checkstudent == FALSE) { //check whether valid student
...
$disabled = '';
$errormsgarray = array(); //error message array
$checkdupcourse = array(); //check duplicate course


$currentbids = $BidDAO->getBidFromUserByRoundNo($_SESSION['userid'], $active->roundNo); //gets current bids according to active round
if (count($currentbids) >= 5 && !isset($_SESSION['isUpdate'])) { //checks if session  limit reached and not update bid
    $errormsgarray[] = 'Maximum number of biddable modules is 5';
    $disabled = 'disabled';
}
if (isset($_SESSION['bidcart'])) {
    $bidcart = $_SESSION['bidcart']; //checks whether if there is a bid cart
} else {
    $bidcart = array();
}
if (count($currentbids) + count($bidcart) >  5 && !isset($_SESSION['isUpdate'])) { //have more than 5 bids and is not update bid
    $errormsgarray[] = 'Maximum number of biddable modules is 5';
    $disabled = 'disabled';
}
if (count($bidcart) > 5 && !isset($_SESSION['isUpdate'])) { //already have more than 5 bids in bid cart
    $errormsgarray[] = 'Maximum number of biddable modules is 5';
    $disabled = 'disabled';
}
$userenrolled = $EnrolledDAO->getEnrolledFromUser($_SESSION['userid']); //checks for enrolled mods
if ((count($userenrolled) > 5 && !isset($_SESSION['isUpdate'])) || (count($userenrolled) + count($bidcart) > 5 && !isset($_SESSION['isUpdate']))) { //enrolled in 5 mods or enrolled+bidding is more than 5 mods
    $errormsgarray[] = 'Maximum number of enrolled and biddable modules is 5';
    $disabled = 'disabled';
}
foreach ($bidcart as $anitem) {
    $checkdupcourse[] = $anitem['course']; //add course code to checkdupcourse
}
if (count($checkdupcourse) !== count(array_unique($checkdupcourse))) { //check whether more than 1 section is bidded for
...
if (isset($_SESSION['isUpdate'])) {
    $bidded = $_SESSION['bidcart'][0]['amount'];//get the prev amount for duplicate  mod
} else {
    $bidded = 0;
}

$errormsgarray = array_unique($errormsgarray); //get unique error message
?>
...
<head>
    ...
    <script>        
        $(document).ready(function(){ 
            var remaining = <?php echo $_SESSION['edollar'] ?>; //get remaining e$
            $('#remaining').text(remaining.toFixed(2)) //2 dec
            $("#checkoutform").on('input', '#bid-total',function(){ //everytime there is a input in the checkout form
                var remaining = <?php echo $_SESSION['edollar'] ?>; 
                var bidded = <?php echo $bidded ?>;
                $('#checkoutform #bid-total').each(function(){ //get each bidding amount
                    var eachVal = $(this).val();
                    if($.isNumeric(eachVal)){  
                        remaining -= parseFloat(eachVal);
                        remaining += bidded; //for update bid
                    }
                });
            $('#remaining').text(remaining.toFixed(2));
            if (remaining < 0) { //check for negative e$
    ...
    <div class="container-fluid d-flex flex-column">
        ...
        <div class="row justify-content-center">
            <div class="col-8 col-m-12 border rounded pb-3">
                <?php
                if (count($errormsgarray) > 0) { //check if any error msg
                    echo "
                ...
                <form method="POST" id="checkoutform" action="student-processcheckout.php">
                    <div class="row container-fluid m-0 pt-3">
                        <?php
                        if (empty($_SESSION['bidcart'])) { //if bid cart empty
                            echo "
                                ...
                                <tbody>';
                                    ...
                                    foreach ($_SESSION['bidcart'] as $abid) {
                                        ...
                                        <tr>
                                            ...
                                            } else { //onchange = will auto 2 decimal, min= 10 e$
                                            echo "
                    ...
                    <div class="row container-fluid d-flex flex-row-reverse m-0">
                        <h4>Total Bid Amount: e$<output id="result"></output></h4> <!--from the calc-->
                    </div>
                    <div class="row container-fluid d-flex flex-row-reverse m-0">
